...
class OKXAcount:
    ...
    def post_to_okx_api(self, ext_user_id, ext_user_name, query_id):
        ...
        response = self.session.post(
            url, json=payload, headers=self.headers(query_id)
        )
    ...
    def assess_prediction(self, ext_user_id, predict, query_id):
        ...
        payload = {'extUserId': ext_user_id, 'predict': predict, 'gameId': 1}
        response = self.session.post(
            url, json=payload, headers=self.headers(query_id)
        )
        if 'data' in response.json():
    ...
    def check_daily_rewards(self, ext_user_id, query_id):
        ...
        try:
            ...
            daily_check_in_task = next(
                (task for task in tasks if task['id'] == 4), None
            )
            if daily_check_in_task:
                if daily_check_in_task['state'] == 0:
                    self.log('Start checkin ... ')
                    self.perform_check_in(
                        ext_user_id, daily_check_in_task['id'], query_id
                    )
                else:
    ...
    def perform_check_in(self, ext_user_id, task_id, query_id):
        ...
        payload = {'extUserId': ext_user_id, 'id': task_id}
        try:
            self.session.post(
                url, json=payload, headers=self.headers(query_id)
            )
    ...
    def wait_with_countdown(self, seconds):
        for i in range(seconds, -1, -1):
            print(
                f'\r===== Completed all accounts, waiting {i} seconds to continue the loop =====',
                end='',
                flush=True,
            )
            time.sleep(1)
    ...
    def countdown(self, seconds):
        for i in range(seconds, -1, -1):
            print(
                f'\r[*] Wait {i} Seconds to continue ...', end='', flush=True
            )
            time.sleep(1)
        print('')
    ...
    def extract_user_data(self, query_id):
        ...
        return {'extUserId': user['id'], 'extUserName': user['username']}
    ...
    def use_boost(self, query_id):
        ...
        try:
            ...
            if response.json().get('code') == 0:
                ...
                self.log(
                    Fore.RED
                    + f"Error Reload Fuel Tank: {response.json().get('msg')}"
                )
        except Exception as e:
    ...
    def upgrade_fuel_tank(self, query_id):
        ...
        try:
            ...
            if response.json().get('code') == 0:
                ...
                self.log(
                    Fore.RED
                    + f"Upgrade error Fuel Tank: {response.json().get('msg')}"
                )
        except Exception as e:
            self.log(Fore.RED + f'Crazyi: {e}')
    ...
    def upgrade_turbo(self, query_id):
        ...
        try:
            ...
            if response.json().get('code') == 0:
                ...
                self.log(
                    Fore.RED
                    + f"Turbo Charge upgrade errorr: {response.json().get('msg')}"
                )
        except Exception as e:
    ...
    def main(self):
        ...
        while True:
            for name, query_id in config.ACCOUNTS.items():
                ...
                try:
                    ...
                    fuel_tank = next(
                        (boost for boost in boosts if boost['id'] == 2), None
                    )
                    ...
                    turbo = next(
                        (boost for boost in boosts if boost['id'] == 3), None
                    )
                    ...
                    for _ in range(50):
                        response = self.post_to_okx_api(
                            ext_user_id, ext_user_name, query_id
                        )
                        if response and 'data' in response:
                            balance_points = response['data']['balancePoints']
                            self.log(
                                Fore.GREEN + f'balancePoints: {balance_points}'
                            )

                            assess_response = self.assess_prediction(
                                ext_user_id, config.prediction, query_id
                            )
                            if assess_response and 'data' in assess_response:
                                ...
                                calculated_value = (
                                    assess_data['basePoint']
                                    * assess_data['multiplier']
                                )
                                self.log(
                                    Fore.MAGENTA
                                    + f"Result: {result} x {assess_data['multiplier']}! Balance: {assess_data['balancePoints']}, Receive: {calculated_value}, Old price: {assess_data['prevPrice']}, Current price: {assess_data['currentPrice']}"
                                )

                                reload_fuel_tank = next(
                                    (
                                        boost
                                        for boost in boosts
                                        if boost['id'] == 1
                                ...
                                if (
                                    assess_data['numChance'] <= 0
                                    and reload_fuel_tank
                                    and reload_fuel_tank['curStage']
                                    < reload_fuel_tank['totalStage']
                                ):
    ...
    def upgrade_if_possible(
        self, ext_user_id, ext_user_name, query_id, boost, boost_name
    ):
        balance_response = self.post_to_okx_api(
            ext_user_id, ext_user_name, query_id
        )
        if balance_response and 'data' in balance_response:
            ...
            if (
                ...
                and balance_points >= boost['pointCost']
            ):
                self.upgrade_boost(query_id, boost['id'], boost_name)
        else:
            self.log(
                Fore.RED + f'Failed to get balance data for {boost_name}.'
            )
    ...
    def upgrade_boost(self, query_id, boost_id, boost_name):
        ...
        if response.json().get('code') == 0:
            ...
            self.log(
                Fore.RED
                + f'Upgrade error {boost_name}: {response.json().get("msg")}'
            )
