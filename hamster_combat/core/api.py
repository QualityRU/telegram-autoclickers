...
class HamsterKombatAccount:
    ...
    def HttpRequest(
        ...
        try:
            ...
            if ignore_errors:
                return None
            log.error(
                f'{w.rs}{w.g}[{self.account_name}]{w.rs}: ✖ Error: {w.r}{e}'
            )
            self.SendTelegramLog(
                f'{w.rs}{w.g}[{self.account_name}]{w.rs}: ✖ Error: {e}',
                'http_errors',
            )
            return None
    ...
    def ClaimDailyCombo(self):
        ...
        existsUpgrades = upgradesResponse.get('dailyCombo', {}).get(
            'upgradeIds', []
        )
        upgradesForBuy = [
            card
            for card in availableUpgrades
            if card['id'] not in existsUpgrades
        ]
    ...
    def BuyCard(self, card):
        ...
        log.info(
            f'{w.rs}{w.g}[{self.account_name}]{w.rs}: ✔ Card bought successfully'
        )
        time.sleep(3)
    ...
    def BuyBestCard(self):
        log.info(
            f'{w.rs}{w.g}[{self.account_name}]{w.rs}: 🃏 Checking for best card.'
        )
        time.sleep(2)
        upgradesResponse = self.UpgradesForBuyRequest()
        if upgradesResponse is None:
    ...
    def StartMiniGame(self, AccountConfigData, AccountID):
        ...
        for game in minigames:
            ...
            if (
                game['id'] == 'Candles'
                and game['remainSecondsToNextAttempt'] > 0
            ):
                log.info(
            ...
            score_cipher = 2 * (
                number + (score_per_game[responseGameData['id']])
            )
    ...
    def GetPlayGroundGameKey(self, promoData):
        ...
        if promoData.get('useNewApi'):
            ...
            response = self.HttpRequest(
                url, headers_post, 'POST', 200, payload
            )
            if response is None:
                log.error(
                    f"{w.rs}{w.g}[{self.account_name}]{w.rs}: Unable to get {w.bb}{promoData['name']}{w.rs} key."
    ...
    def Start(self):
        log.info(
            f'{w.rs}{w.g}[{self.account_name}]{w.rs}: 🌌 Starting account.'
        )

        log.info(
            f'{w.rs}{w.g}[{self.account_name}]{w.rs}: 📡 Getting account IP.'
        )
        ...
        log.info(
            f'{w.rs}{w.g}[{self.account_name}]{w.rs}: 🛸 Getting account data.'
        )
        ...
        log.info(
            f'{w.rs}{w.g}[{self.account_name}]{w.rs}: 🎯 Getting account tasks.'
        )
        ...
        log.info(
            f'{w.rs}{w.g}[{self.account_name}]{w.rs}: 👕 Getting account skins.'
        )
        ...
        if self.config['auto_tap']:
            log.info(
                f'{w.rs}{w.g}[{self.account_name}]{w.rs}: 👇 Starting to tap.'
            )
            ...
            if streak_days['isCompleted'] == True:
                ...
                if days == 7:
                    ...
                    if buyResponse is None:
                        ...
                        log.info(
                            f' 👌 Successfully obtained weekly reward skin'
                        )
