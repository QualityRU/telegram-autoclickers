...
class HamsterKombatAccount:
    ...
    def ClaimDailyCombo(self):
        if not self.GetConfig('auto_claim_daily_combo', False):
            log.info(
                f'[{self.account_name}] Auto claim daily combo are disabled.'
            )
            return
        ...
        if comboCards is None:
            log.error(
                f'[{self.account_name}] Combo cards response is incorrect.'
            )
            return
    ...
    def ClaimDailyComboRequest(self):
        ...
        if 'error_code' in response:
            ...
            log.info(
                f'[{self.account_name}] Daily combo successfully claimed.'
            )
            return True
